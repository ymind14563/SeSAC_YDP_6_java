...
// LinkedList 는 노드: 이전 노드 주소(prev) + [ 요소 ] + 다음 노드 주소(next)로 이루어져 있음 (정확한 순서가 아니라 산재되어 있음)
// 각 노드의 prev 에는 이전 노드의 주소 값이, next 에는 다음 노드의 주소 값이 들어 있음
// Head 는 첫 번째 노드를 가리키며, Tail 은 마지막 노드를 가리킴
